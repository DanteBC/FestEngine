...
- Добавлена возможность перемотки трека щелчком по прогресс-бару
- Добавлена возможность выбора вывода для VLC (directsound на винде помогает убрать баг отсутствия музыки в треке при наличии фоновой музыки)
- Добавлена возможность отправки POST запросов о действиях по программе фестиваля (очень полезно для актуализации состояния онлайн программы фестиваля)
- Добавлен простенький локальный API сервер для внешнего опроса текущего номера программы
- Добавлена темная тема (вместе с переездом на wxPython 4.3.1) и иконка
